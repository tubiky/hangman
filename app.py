...
def main():
    ...
    click_sound = pygame.mixer.Sound(os.path.join(os.getcwd(), 'sounds/clicked.wav'))
...
# 입력용 키패드를 그리는 function
    def drawKeypad():
        ...
        for i, j in zip(alphabet, order):
            i = small_font.render(alphabet[j], True, (0, 0, 0))
            screen.blit(i, (KEYPADS[j].x+1, KEYPADS[j].y+1))
            ...
            if event.type == pygame.KEYDOWN:
                if active:
                    if event.key == pygame.K_F1:
                        answer, underbar = randomize_answer(WORDS)
                        checkedAlphabets = set()
                        ...
                        text += event.unicode
                        displayedAnswer = spellCheck(text, answer, WORDS)
                        text = ''
